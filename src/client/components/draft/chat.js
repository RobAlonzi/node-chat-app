...
export class Chat extends ContainerBase{
	constructor(props){
		...
		this._messages$ = Observable.merge(
			...
			this._server.on$("chat:added:fail").filter(m => m).map(message => <ChatMessage key={uniqueId("chat-add-fail")} data={{error: message.clientMessage}} type="chat-add-fail" author="system" />)
		);
